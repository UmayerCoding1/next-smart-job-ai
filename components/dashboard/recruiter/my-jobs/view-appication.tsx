      ...
      <div className="flex  justify-between py-4 mb-2 border-b border-neutral-300">
        <div>
          <h1 className="text-xl font-semibold">
            Applications for {selectedJobAppilications.title}
          </h1>
          <p className="text-neutral-500 text-sm font-medium">
            {selectedJobAppilications.totalAppications} total applications
          </p>
        </div>
      ...
      <div className="max-h-[400px] overflow-auto scrollbar-hide mt-10">
        {JobAppilications.length === 0 ? (
          ...
          JobAppilications?.map((appication: IApply, index: number) => (
            <div
              key={index}
              className="w-full h-[110px] flex items-center justify-between border border-neutral-300 p-2 mb-2 rounded-lg relative"
            >
              <div className="flex gap-2">
                <Image
                  src={
                    appication.applicant.avatar ||
                  ...
                  className={`w-32 h-32 rounded-lg object-cover ${
                    !appication.applicant.avatar && "border border-neutral-300"
                  }`}
                />
                ...
                <div>
                  ...
                  <p className="text-neutral-500 text-sm font-medium">
                    {appication.countryCode?.slice(0, 3)}
                    {appication.phone}
                  </p>

                  <p className="text-neutral-600 text-xs mt-5 font-medium ">
                    <span>Applied on</span>{" "}
                    {new Date(appication.appliedAt).toLocaleDateString("en-US")}
                  </p>
                </div>
              </div>
