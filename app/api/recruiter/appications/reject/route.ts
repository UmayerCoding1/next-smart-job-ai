...
export async function PUT(req: NextRequest) {
   try {
    ...
    const applications = await Application.updateMany({recruiter: verifiedUser.userId}, {status: 'rejected',isRead: true});

    if(!applications) return NextResponse.json({ message: "Application not found", success: false }, { status: 500 });

    return NextResponse.json({ message: "All applications as rejected", success: true }, { status: 200 });
